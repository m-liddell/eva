...
export function OnboardingTour({ isOpen, onClose, onComplete, currentContext = 'home' }: OnboardingTourProps) {
  ...
  // Get proof-of-concept user data for personalization
  const getUserData = () => {
  ...
  const pocUserData = getUserData();
  const userName = pocUserData?.name || 'Teacher';

  const tourSteps: TourStep[] = [
    {
      id: 'welcome',
      title: `Welcome to EVA, ${userName}!`,
      description: `Your Educational Virtual Assistant is here to help you plan, teach, and assess with confidence. ${pocUserData ? 'This is a demonstration environment where you can explore all features safely. ' : ''}Let\'s walk through the complete workflow step by step.`,
      target: 'welcome',
    ...
    },
    {
      ...
      description: `Start by creating your weekly timetable. You'll add lessons, group them into themes, edit individual activities, and approve weeks for teaching. ${pocUserData ? 'Feel free to experiment - this is a safe demo environment!' : 'This is where your teaching journey begins.'}`,
      target: 'plan-section',
    ...
    },
    {
      ...
      description: `That's the core workflow: Plan → Teach → Assess. You can also access training, reports, settings, and the lesson library from the main menu (click the menu icon in the top left). ${pocUserData ? 'Remember, this is a demonstration - explore freely and see what EVA can do for your teaching!' : 'Ready to begin your teaching journey?'}`,
      target: 'complete',
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            {currentTourStep.icon}
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-gray-800">{currentTourStep.title}</h2>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between p-4 sm:p-6 border-t border-gray-200 gap-3 sm:gap-0">
          ...
          <div className="flex items-center gap-3 order-1 sm:order-2">
            <button
              ...
              className="flex items-center gap-2 px-3 sm:px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
            ...
            <button
              onClick={handleNext}
              className="flex items-center gap-2 px-4 sm:px-6 py-2 bg-[#FFC83D] text-white rounded-lg hover:bg-[#E6B434] transition-colors font-medium text-sm"
            >
