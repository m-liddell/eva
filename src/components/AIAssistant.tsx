...
export function AIAssistant({ onClose }: AIAssistantProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-25">
      <div className="bg-white w-full max-w-4xl mx-4 rounded-lg shadow-lg max-h-[90vh] overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-[#FFC83D]" />
            <h2 className="text-base sm:text-lg font-bold">Educational Virtual Assistant</h2>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowPrompts(!showPrompts)}
              className="hidden sm:block px-3 py-1 text-sm bg-[#FFF9E7] text-[#FFC83D] rounded-lg hover:bg-[#FFC83D]/10 transition-colors"
            >
        ...
        <div className="flex flex-col sm:flex-row h-[500px] sm:h-[600px]">
          {/* Prompts Panel */}
          {(showPrompts || window.innerWidth < 640) && (
            <div className="w-full sm:w-1/2 border-b sm:border-b-0 sm:border-r border-gray-200 p-4 overflow-y-auto max-h-[200px] sm:max-h-none">
                  ...
                  <div key={category.category} className="space-y-2">
                    ...
                    <div className="space-y-1 ml-2">
                      {category.prompts.map((prompt, index) => (
                        ...
                        >
                          <div className="font-medium text-gray-800 mb-1 text-xs sm:text-sm">{prompt.title}</div>
          ...
          {/* Chat Area */}
          <div className={`${(showPrompts && window.innerWidth >= 640) ? 'w-1/2' : 'w-full'} flex flex-col`}>
            {/* AI Disclaimer */}
            ...
            <div className="p-4 border-t border-gray-200">
              ...
              <div className="sm:hidden mb-3">
                <button
                  onClick={() => setShowPrompts(!showPrompts)}
                  className="w-full px-3 py-2 text-sm bg-[#FFF9E7] text-[#FFC83D] rounded-lg hover:bg-[#FFC83D]/10 transition-colors"
                >
                  {showPrompts ? 'Hide Prompts' : 'Show Prompts'}
                </button>
              </div>
