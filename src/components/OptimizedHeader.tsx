...
export function OptimizedHeader({ onMenuClick, onAIClick, lessonInfo }: HeaderProps) {
  ...
  const displayName = pocUserData?.name || 'Teacher';

  const getPhaseIcon = () => {
    if (!lessonInfo?.phase) return null;
    
    const icons = {
      'Starter': <Brain className="w-4 h-4 text-blue-600" />,
      'Main': <BookOpen className="w-4 h-4 text-[#FFC83D]" />,
      'Plenary': <CheckCircle2 className="w-4 h-4 text-green-600" />
    };
    return icons[lessonInfo.phase] || <BookOpen className="w-4 h-4 text-[#FFC83D]" />;
  };
  ...
  const getPhaseColor = () => {
    ...
    const colors = {
      ...
      'Main': 'bg-[#FFF9E7] text-[#FFC83D] border-[#FFC83D]/20',
      'Plenary': 'bg-green-100 text-green-700 border-green-200'
    };
    return colors[lessonInfo.phase] || 'bg-gray-100 text-gray-700 border-gray-200';
  };
    ...
    <header className="bg-white border-b border-gray-200">
      <div className="max-w-[1400px] mx-auto px-4 h-12 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Menu className="w-5 h-5 text-gray-600 cursor-pointer" onClick={onMenuClick} />
          <Link href="/" className="text-lg font-bold text-[#FFC83D]">EVA</Link>
            ...
            <div className="hidden md:flex items-center gap-4 ml-4">
              ...
              {lessonInfo.subject && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs font-medium text-gray-700">{lessonInfo.subject}</span>
                </div>
              )}
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onAIClick} className="w-7 h-7 bg-[#FFCFCF] rounded-full flex items-center justify-center">
