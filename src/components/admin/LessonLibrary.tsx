...
import { SherlockHolmesUnitViewer } from '../../components/SherlockHolmesUnitViewer';
...
export default function LessonLibrary() {
  ...
  const [showSherlockUnitViewer, setShowSherlockUnitViewer] = useState(false);
  ...
  const fetchActivities = async () => {
    try {
      ...
      // Use only enhanced activities from local data
      setActivities([...allEnhancedActivities]);
    } catch (err) {
    ...
    <div className="min-h-[calc(100vh-48px)] bg-[#F8F9FA]">
      <div className="max-w-[1400px] mx-auto px-4 py-6">
        ...
        <div className="mb-8">
          ...
          <div 
            onClick={() => setShowSherlockUnitViewer(true)}
            className="bg-white rounded-lg border border-gray-200 p-6 mb-6 cursor-pointer hover:shadow-md transition-shadow"
          >
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Sherlock Holmes: Detective Fiction and Deductive Reasoning</h3>
                <p className="text-gray-600 leading-relaxed">
                  Complete 12-lesson unit exploring detective fiction through Arthur Conan Doyle's Sherlock Holmes stories, developing analytical thinking, deductive reasoning skills, and sophisticated writing techniques for compelling mystery narratives
                </p>
              </div>
              <div className="flex items-center gap-3 ml-6">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleFavorite('sherlock-holmes-unit');
                  }}
                  className="p-2 text-gray-400 hover:text-red-500 transition-colors"
                  title={favorites.includes('sherlock-holmes-unit') ? "Remove from favorites" : "Add to favorites"}
                >
                  <Heart className={`w-5 h-5 ${favorites.includes('sherlock-holmes-unit') ? 'fill-current text-red-500' : ''}`} />
                </button>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowSherlockUnitViewer(true);
                  }}
                  className="p-2 text-gray-400 hover:text-gray-600 transition-colors"
                  title="View unit details"
                >
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Interactive lessons 1-6 available
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Modals */}
        {showUploadModal && (
          <LessonUploadModal onClose={() => setShowUploadModal(false)} />
        )}
        ...
        {viewingActivity && (
          <ActivityDetailsModal
            ...
            onOpenInteractive={() => handleOpenInteractiveLesson(viewingActivity)}
          />
        )}

        {showDystopianUnitViewer && (
          <DystopianUnitViewer onClose={() => setShowDystopianUnitViewer(false)} />
        )}

        {showSherlockUnitViewer && (
          <SherlockHolmesUnitViewer onClose={() => setShowSherlockUnitViewer(false)} />
        )}
