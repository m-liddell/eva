...
export default function TeachLesson() {
  ...
  useEffect(() => {
    // In Next.js, we need to get lesson data from URL params or other source
    // For now, we'll use a default lesson
    const lessonData = null; // This would come from URL params or API
    if (!lessonData) {
    ...
    if (lessonData.theme === 'Dystopian Fiction') {
      ...
      if (interactiveRoute) {
        navigate(interactiveRoute, { replace: true });
        return;
      }
    }
