                      ...
                      <div key={index} className={`p-4 ${themeColors.itemBg} rounded-lg border ${themeColors.border} hover:shadow-md transition-all cursor-pointer`}>
                        ...
                        {element.element_type === 'hands_on_activity' && (
                          <div className="mt-4 space-y-3">
                            {/* Sherlock Holmes Detective Objects */}
                            {element.title.includes('Mystery Object') && (
                              <div className="grid grid-cols-3 gap-3">
                                {[
                                  { name: 'Victorian Pocket Watch', image: 'https://images.unsplash.com/photo-1509048191080-d2e72ca020bd?auto=format&fit=crop&q=80&w=300', clues: 'Scratched, engraved, well-worn' },
                                  { name: 'Modern Smartphone', image: 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&q=80&w=300', clues: 'Detective apps, case notes' },
                                  { name: 'Library Books', image: 'https://images.unsplash.com/photo-1481627834876-b7833e8f5570?auto=format&fit=crop&q=80&w=300', clues: 'Bookmarks, margin notes' }
                                ].map((object, objIndex) => (
                                  <div key={objIndex} className="relative group">
                                    <img 
                                      src={object.image}
                                      alt={object.name}
                                    ...
                                    <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                                      <div className="text-white text-center">
                                        <div className="text-xs font-medium">{object.name}</div>
                                        <div className="text-xs">{object.clues}</div>
                                      </div>
                                    </div>
                                  </div>
                                ))}
                            ...
                            {/* Original dystopian images for other activities */}
                            {!element.title.includes('Mystery Object') && (
                            <div className="grid grid-cols-3 gap-3">
