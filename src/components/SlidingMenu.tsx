...
export function SlidingMenu({ isOpen, onClose }: SlidingMenuProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div className={`absolute top-0 left-0 w-64 sm:w-72 h-full bg-white shadow-lg transform transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <div className="space-y-1">
                  {item.subItems ? (
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-gray-100 text-gray-700 text-left"
                    >
                    ...
                    <Link
                      href={item.path}
                      onClick={onClose}
                      className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 text-gray-700 text-left"
                    >
                      <item.icon className="w-5 h-5" />
                      <span>{item.label}</span>
                    </Link>
                    ...
                    <ul className="ml-8 space-y-1">
                      {item.subItems.map((subItem) => (
                        <li key={subItem.path}>
                          <Link
                            href={subItem.path}
                            className="block px-3 py-2 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-lg text-left"
                            onClick={onClose}
                          >
                            {subItem.label}
                          </Link>
            ...
            <ul className="space-y-2">
              {adminItems.map((item) => (
                <li key={item.label}>
                  <Link
                    href={item.path}
                    className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 text-gray-700 text-left"
                    onClick={onClose}
                  >
                    <item.icon className="w-5 h-5" />
                    <span>{item.label}</span>
